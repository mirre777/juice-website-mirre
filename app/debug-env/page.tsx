...
function FirebaseTest() {
  ...
  const testFirebase = async () => {
    ...
    try {
      // Call our API route instead of importing Firebase directly
      const response = await fetch("/api/debug-firestore")
      const data = await response.json()

      if (data.success) {
        setTestResult(
          `✅ Firebase connection successful! Found ${data.readTest.documentsFound} documents in debug-test collection.`,
        )
      } else {
        setTestResult(`❌ Firebase test failed: ${data.error}`)
      }
    } catch (error) {
