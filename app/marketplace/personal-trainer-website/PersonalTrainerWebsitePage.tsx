...
try {
  if (contentType.includes("application/json")) {
    ...
    // Wrap text in an object so downstream handlers can read a message
    return { ok: response.ok, status: response.status, body: text ? { message: text } : {} }
  }
} catch (err) {
  // As an ultimate fallback, return status + empty body
...
const handleFormSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    const { ok, status, body } = await parseApiResponse(response)
