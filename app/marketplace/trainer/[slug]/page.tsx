...
export default function TrainerPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchTrainer = async () => {
      try {
        // Try to fetch trainer by slug (could be custom slug or ID)
        const response = await fetch(`/api/trainer/by-slug/${slug}`)
        ...
        if (data.success && data.trainer) {
          // Check if we need to redirect for SEO purposes
          if (data.shouldRedirect && data.canonicalUrl) {
            // Use window.location.replace for SEO-friendly redirect
            window.location.replace(data.canonicalUrl)
            return
          }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const response = await fetch(`/api/trainer/content/${trainer.id}`, {
        method: "PUT",
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <head>
        <link
          rel="canonical"
          href={`${typeof window !== "undefined" ? window.location.origin : ""}/marketplace/trainer/${
            trainer.settings?.customSlug || trainer.id
          }`}
        />
      </head>
